...
export const Contact = ({ onEnquire }: Props) => (
  <main className="pt-28 pb-20">
    ...
    <section className="container grid md:grid-cols-2 gap-6 mb-10">
      {[
        ...
        <div key={label} className="flex gap-4 p-6 rounded-2xl bg-card border border-border shadow-soft">
          ...
          <div>
            ...
            {href ? (
              <a href={href} className="font-serif text-lg text-primary hover:text-primary-glow transition-smooth">{value}</a>
            ) : (
    ...
    {/* WhatsApp + Enquire */}
    <section className="container grid md:grid-cols-2 gap-4 mb-12">
      <a
        href={`https://wa.me/${whatsappNumber}?text=${whatsappMsg}`}
        target="_blank"
        rel="noopener noreferrer"
        className="flex items-center justify-center gap-3 h-14 rounded-2xl bg-whatsapp text-primary-foreground font-semibold shadow-card hover:brightness-110 transition-smooth"
      >
        <MessageCircle className="w-6 h-6" /> Chat on WhatsApp
      </a>
      <Button variant="navy" size="xl" onClick={onEnquire} className="h-14 rounded-2xl">
        Send an Enquiry
      </Button>
    </section>
