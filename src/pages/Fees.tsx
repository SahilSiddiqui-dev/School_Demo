    ...
    <main className="py-20 md:py-28 bg-background relative overflow-hidden">
      ...
      <div className="container relative">
        ...
        <ScrollReveal className="max-w-2xl mx-auto">
          <div className="bg-card border border-border/70 rounded-3xl shadow-soft p-6 md:p-8 premium-grid relative overflow-hidden">
            <label className="block text-sm font-semibold text-primary mb-3">Select Class</label>
            <Select value={grade} onValueChange={setGrade}>
              <SelectTrigger className="h-12 rounded-xl border-border/70 focus:ring-ring">
            ...
            {!plan && (
              <p className="text-muted-foreground text-sm mt-8 text-center animate-pulse">
            ...
            {plan && (
              <div className="mt-8 animate-in fade-in slide-in-from-bottom-2 duration-300">
                ...
                <div className="rounded-2xl border border-border/75 overflow-hidden shadow-soft">
                  {plan.rows.map((r, i) => (
                    <div
                      key={r.label}
                      className={`flex items-center justify-between px-5 py-3.5 text-sm transition-colors border-b border-border/30 last:border-b-0 ${
                  ...
                  <div className="flex items-center justify-between px-5 py-4 bg-primary text-primary-foreground">
                    <span className="font-semibold flex items-center gap-1.5">
                      <IndianRupee className="w-4 h-4 text-secondary" /> Total (first year)
                    </span>
                    <span className="font-bold text-xl tabular-nums text-secondary">{formatINR(total)}</span>
                  </div>
                </div>
                ...
                <Button variant="hero" size="lg" className="w-full h-12 text-base rounded-xl" onClick={onEnquire}>
